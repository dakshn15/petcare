...
export const createReview = async (req, res, next) => {
  try {
    const { name, email, rating, text, isApproved } = req.body;
    const isAdminUser = req.user && req.user.role === 'admin';

    // Check if authenticated user already submitted a review (skip for admins)
    ...
    // Check by email to prevent double posting from the same email address (skip for admins)
    if (!isAdminUser) {
    ...
    const review = await Review.create({
      ...
      rating,
      text,
      isApproved: isAdminUser ? (isApproved !== undefined ? isApproved : true) : false
    });

    logActivity(req.user?._id, 'CREATE_REVIEW', 'Review', review._id.toString(), `New review from ${name}`, req.ip);

    successResponse(res, review, isAdminUser ? 'Review created successfully.' : 'Review submitted! It will appear after admin approval.', 201);
  } catch (error) {
    next(error);
  }
};
...
export const updateReview = async (req, res, next) => {
  try {
    ...
    if (email) review.email = email.toLowerCase();
    ...
    logActivity(req.user._id, 'UPDATE_REVIEW', 'Review', review._id.toString(), `Updated review from ${review.name}`, req.ip);
